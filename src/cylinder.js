...
class Cylinder {
    constructor(gl, baseR = 1, topR = 1, h = 1, sectors = 36, stacks = 1, smooth = true) {
        this.gl = gl;
        if (!gl) log("[WARNING] Cylinder.contructor requires GL context as a param.");
    ...
    buildVerticesSmooth() {
        ...
        for (i = 0, k = baseVertexIndex + 1; i < this.sectorCount; ++i, ++k) {
            if (i < this.sectorCount - 1) this.addIndices(kk, baseVertexIndex, k + 1, k);
            // last triangle
            else this.addIndices(kk, baseVertexIndex, baseVertexIndex + 1, k);
            kk += 3;
        }

        for (i = 0, k = topVertexIndex + 1; i < this.sectorCount; ++i, ++k) {
            if (i < this.sectorCount - 1) this.addIndices(kk, topVertexIndex, k, k + 1);
            else this.addIndices(kk, topVertexIndex, k, topVertexIndex + 1);
            kk += 3;
        }
        ...
        for (i = 0; i < this.stackCount; ++i) {
            ...
            for (j = 0; j < this.sectorCount; ++j, ++vi1, ++vi2) {
                ...
                for (
                    k = 0;
                    k < 4;
                    ++k // same normals for all 4 vertices
                ) {
